...
public class Book extends DBconnect {
	...
	public G_Opened_Movie current_movie_info() { // 현재 상영 영화 정보
		...
		return opened_movie().get(movieStartTime - 1);// 위에서 인덱스를 찾았으니 그 인덱스에서
														// -1한값이 내가 선택한 위치
	}
		...
		outer: while (true) {
			...
			case 3: // 예매 조회
				...
				for (G_Book G_book : view_book(phone_number)) {// 전화번호를 통해
					...
					System.out.println(String.format("%-10s %-10s %-10s %-6s %-20s %-20s", booknumber, start, bookseat,
							theater, date, title));
				...
				System.out.println();
				System.out.println("Enter를 입력하면 메뉴화면으로 돌아갑니다.");
				sc.nextLine();
				// System.out.println("메뉴화면으로 돌아갑니다.");
			...
			case 4: // 예매 취소
				int i = 1;
				System.out.println("    예매번호\t  상영시작시간\t  영화제목\t\t상영관\t예매한날짜\t     예약좌석");
				for (G_Book G_book : view_book(phone_number)) {// 예매내용을 불러온후
																// 인덱스를
																// 입력하게해서
																// 정보를 찾음
					System.out.printf("%2d. %-15s %-15s %-20s %-6s %-20s %-4s\n", i, G_book.getB_number(),
							G_book.getO_starttime().substring(0, 5), G_book.getMv_title(), G_book.getT_theater(),
							G_book.getB_date(), G_book.getB_bookseat());
					i++;
				}
				...
				G_Book gbook = view_book(phone_number).get(delNumber - 1);// 번호는
																			// 1부터
																			// 시작하니
																			// -1의
																			// 값이
																			// 삭제할
																			// 행임
				int mf_openseat = 1 + find_open_seat(gbook.getO_starttime(), gbook.getMv_title(), gbook.getT_theater());// 예매취소를
																														// 했으니
																														// 사용가능좌석에
																														// +1을
																														// 해야함

				view_open_seat(mf_openseat, gbook.getO_starttime(), gbook.getT_theater(), gbook.getMv_title());// 좌석값을
																												// 넣음

				delete_book(gbook.getB_number());// 관련 내용을 삭제함
				break;
			case 5:
				break outer;
			}
